...
class NikonScraper extends BaseScraper {
    constructor() {
        super('Nikon', 'https://www.nikon.com');
    }

    async scrape() {
        console.log('  🔍 抓取 Nikon 新闻...');
        
        try {
            const html = await this.fetch('https://www.nikon.com/company/news/');
            
            const newsItems = this.extractNewsItems(html);
            
            for (const item of newsItems.slice(0, 5)) {
                if (this.isCameraRelated(item)) {
                    const cameras = this.extractCameraInfo(item.title + ' ' + item.content, 'nikon');
    ...
    extractNewsItems(html) {
        const items = [];
        const newsRegex = /<div[^>]*class="[^"]*news[^"]*"[^>]*>[\s\S]*?<\/div>/gi;
    ...
    isCameraRelated(item) {
        const keywords = ['z ', 'nikon', 'mirrorless', 'camera', 'lens', 'z mount'];
        const text = (item.title + ' ' + item.content).toLowerCase();
        return keywords.some(k => text.includes(k));
    }
}

module.exports = NikonScraper;
